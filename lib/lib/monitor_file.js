        ...
        this.fdTailer.read(function() {
          if (this.fdTailer) {
            ...
            if (last_data) {
              ...
              var fd = this.fdTailer.fd;
              fs.read(fd, buffer, 0, last_data_length, this.fdTailer.current_index - last_data_length, function(err, bytesRead, buffer) {
                if (err) {
                  return this.emit('error', err);
                }
                if (bytesRead === last_data_length && last_data === buffer.toString(this.buffer_encoding, 0, last_data_length)) {
                  logger.debug('Event changed received, but no data change and last data match', this.filename, 'fd', fd);
                }
                else {
                  logger.info('Event changed received, but no data change and last data does not match.', 'Restarting reading', this.filename, 'at 0 fd', fd);
                  this.restart(0);
                }
              }.bind(this));
            }
            ...
            logger.debug('Event change received, but file', this.filename, 'renamed');
          }
        }.bind(this));
      }
      else {

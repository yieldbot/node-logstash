...
function OutputKafkaRiver() {
  ...
  this.mergeConfig({
    name: 'Kafka River',
    optional_params: ['index_prefix', 'default_type', 'date_format', 'type_field', 'id_field'],
    default_values: {
...
OutputKafkaRiver.prototype.formatPayload = function(data, callback) {
  var date = moment.utc();
  if (data['@timestamp']) {
    date = moment.utc(data['@timestamp']);
  }
  ...
  else if (this.type_field && data[this.type_field]) {
    doc.type = data[this.type_field];
  }
  if (this.id_field && data[this.id_field]) {
